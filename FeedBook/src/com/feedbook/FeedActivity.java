...
public class FeedActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SimpleAdapter adapter = new SimpleAdapter(this, listarFeeds(),
				R.layout.feed, de, para);
	...
	public List<Map<String, Object>> listarFeeds() {
		...
		for (int i = 0; i < cursor.getCount(); i++) {
			...
			feed.put("nomeGrupo", cursor.getString(0));
			feed.put("tituloFeed", cursor.getString(1));
			feed.put("detalhe", cursor.getString(2));
			feedList.add(feed);
			cursor.moveToNext();
		}
		cursor.close();
	...
	public void onBackPressed() {
		if (this.lastBackPressTime < System.currentTimeMillis() - 4000) {
			toast = Toast
					.makeText(
							this,
							"Pressione o Botão Voltar novamente para fechar o Aplicativo.",
							4000);
			toast.show();
